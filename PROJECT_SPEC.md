...
> **Status:** Phases 1 through 8 and Phase 9 Milestones 1 through 4 shipped; remaining Phase 9 work planned
...
> Remaining Phase 9 work is tracked in [`docs/phase9-planning/`](./docs/phase9-planning/) and extends the shipped M4 observability foundation.
...
The current implementation already ships Phases 1 through 8 plus Phase 9 Milestones 1 through 4: gateway foundations, load balancing, JWT authentication, Redis-backed rate limiting, retry plus circuit breaking, observability, runtime-readiness management, documentation plus benchmark artifacts, and the first four Chaos Observatory slices (OTel tracing plus overlay bootstrap, the observatory control plane, the full shipped scenario catalog, and the React demo frontend).
...
| Frontend | Current shipped runtime: demo-only React observatory UI under `web/`, not a general admin plane. |
...
- Phase 9 Milestones 1 through 4 are already part of the current runtime: OpenTelemetry export hooks, Prometheus exemplars, the `gateway_circuit_state` gauge, the circuit-breaker reset admin endpoint, the local observatory overlay, the observatory control plane, the shipped scenario catalog, and the demo React frontend.
- The remaining Phase 9 hardening and deploy work beyond M4 is still planned. Current runtime and production behavior remain whatever is documented in [`ARCHITECTURE.md`](./ARCHITECTURE.md), [`PROGRESS.md`](./PROGRESS.md), and [`deploy/README.md`](./deploy/README.md).
